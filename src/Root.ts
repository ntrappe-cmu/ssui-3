...
export class Root {
    ...
    // Do the work needed to set up the event handlers on the associated HTML 
    ...
    // mouse buttion is released outside the canvas, so we never hear about it.  
    ...
    // one.
    // NOTE: Super clever. A common edge case is that user drags mouse outside canvas
    // and releases but we  wont know about this. So we save the previous known button
    // state and compare it against the current button state to see if we missed smth
    protected _lastButtonsState : number = 0;

    // Handler that takes input events from the canvas object.  These get delivered 
